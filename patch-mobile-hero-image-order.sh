...
# ------------------------------------------------------------
# Patch: mobile hero ordering only.
#
# Mobile:
#   Name -> domain -> headline -> portrait -> supporting text/buttons
#
# Desktop:
#   Keeps the existing two-column layout with the portrait on the right.
#
# Nothing else is changed.
#
# Run from the root of michael-portfolio-v3:
#   bash ./patch-mobile-hero-image-order.sh
# ------------------------------------------------------------

if [ ! -f "package.json" ] || [ ! -f "src/components/sections/Hero.tsx" ]; then
